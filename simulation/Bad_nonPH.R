...
sbwbal <- function(X, Tr){
  ...
  sbw.result <- FALSE
  ######The correct SBW output contains 15 items. If the output is incorrect, reduce the search range of bal_grid by one item.
...
###### Compute pairwise RMST difference
...
theta_non=data.frame( ps_theta_1_2=rep(0,s),ps_theta_1_3=rep(0,s),ps_theta_2_3=rep(0,s),
                   ...
                   MDABW_theta_1_2=rep(0,s),MDABW_theta_1_3=rep(0,s),MDABW_theta_2_3=rep(0,s)) #########################新加的
...
for (i in 1:s) {
  ...
  ebal.bal   <- bal.tab(X_new, treat = character[1 + dimension], data = data.matrix, weights = result$ebal+min(result$ebal[which(result$ebal>10^(-7))])*(result$ebal==0)) ################newly added
...
Im_GASMD_mean = list(unad = apply(Im_GASMD[1,,],2,mean),
                     ...
                     MDABW=apply(Im_GASMD[8,,],2,mean)###########新加的
)
...
Im_non_all_sum=data.frame(ps=apply(Im_non_sum[1:3,], 2, sum),
                          ...
                          MDABW=apply(Im_non_sum[22:24,], 2, sum)) ##########newly added
#max
...
write.csv(file = 'Bad_Non_PH_result.csv',x=all_table)
#appendix
#save bias\sd\rmse for all pairwise comparison rather than mean of bias\sd\rmse for all pairwise comparison
#save RMST estimate
write.csv(file = 'Bad_NonPH_rmst_est.csv',x=non_ph_estimate)
#save bias
write.csv(file = 'Bad_NonPH_bias.csv',x=n_bias)
#save sd
write.csv(file = 'Bad_NonPH_sd.csv',x=n_sam)
#save mse
write.csv(file = 'Bad_NonPH_mse.csv',x=n_mse)
#save GASMD for all covariates
Im_GASMD_mean_df<-data.frame(Im_GASMD_mean)
write.csv(file = 'Bad_NonPH_gasmd.csv',x=Im_GASMD_mean_df)