...
weight.esti <- function(X, Tr, delta.space = c(1,1e-2,1e-4)){
  ...
  if(sum(ebal.weight[Tr == 1]) == 0){ebal.weight[Tr == 1] <- rep(1,sum(Tr == 1))}
  if(sum(ebal.weight[Tr == 2]) == 0){ebal.weight[Tr == 2] <- rep(1,sum(Tr == 2))}
  if(sum(ebal.weight[Tr == 3]) == 0){ebal.weight[Tr == 3] <- rep(1,sum(Tr == 3))}
...
#####Imbalance函数用于计算GAMSD,|mean(X_k)-mean(X_k)|/S_k
#####这里将X作了标准化,故只需要计算|mean(X_k)|
...
for (i in 1:s) {
  ...
  ebal.bal   <- bal.tab(X, treat = character[1 + dimension], data = data.matrix, weights = result$ebal)
  cbps.bal   <- bal.tab(X, treat = character[1 + dimension], data = data.matrix, weights = (result$ebal+min(result$ebal[which(result$ebal>10^(-7))])*(result$ebal==0)))
...
wRMSE=data.frame(Unad=sqrt(p_mse_mean[6]),IPW=sqrt(p_mse_mean[1]),
                 ...
                 Energy=sqrt(p_mse_mean[3]),MB=sqrt(p_mse_mean[4]),kernel=sqrt(p_mse_mean[7]),MDABW=sqrt(p_mse_mean[8]))
...
write.csv(file = 'Good_PH_result.csv',x=all_table)

#appendix
#save bias\sd\rmse for all pairwise comparison rather than mean of bias\sd\rmse for all pairwise comparison
#save RMST estimate
write.csv(file = 'Good_PH_rmst_est.csv',x=ph_estimate)
#save bias
write.csv(file = 'Good_PH_bias.csv',x=p_bias)
#save sd
write.csv(file = 'Good_PH_sd.csv',x=p_sam)
#save mse
write.csv(file = 'Good_PH_mse.csv',x=p_mse)
#save GASMD for all covariates
Im_GASMD_mean_df<-data.frame(Im_GASMD_mean)
write.csv(file = 'Good_PH_gasmd.csv',x=Im_GASMD_mean_df)
